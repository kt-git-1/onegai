...
SwiftUI + Firebaseで「おねがいチャリン」というiPhoneアプリのMVPを実装します。

アプリ概要：

おねがいチャリンは、カップルで使えるごほうび貯金箱アプリです。  
...
- 招待された側はリンクまたはコードで招待内容を確認してから、登録/ログインへ進みます。
- 招待リンクは `https://onegai-charin-dev.web.app/invite/{inviteId}` 形式のUniversal Linkを使用します。
- AASAとアプリ未インストール時の案内ページはFirebase Hostingから配信します。
- 招待情報は登録/ログインとプロフィール設定の間も保持します。
- 認証と必須プロフィール設定の完了後にグループ参加を確定します。
- 招待された側はテンプレート適用や新しい招待の作成へ進みません。
